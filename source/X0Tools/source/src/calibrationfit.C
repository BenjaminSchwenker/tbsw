/// The purpose of this script is to do a simultaneous fit of a set of angle
/// distributions, which are taken from the X0_merge.root input file. Each of
/// these distributions
/// should come from an area with a homogenious thickness and material
/// composition. The hole class defined in this script can be used to select the
/// area and give them some attributes
/// like for example the thickness, position and the X0 value in this area. The
/// angle distributions are then fitted by a function which corresponds to the
/// convolution
/// between a function given by a theoretical model for the multiple scattering
/// (highland or moliere model) and a gaussian error function, which depends on
/// the expected telescope angular resolution
/// and a calibration factor. The fit estimates an optimal value for the
/// calibration factor.
///
/// This script can be used by starting root and afterwards typing the command
/// .x calibrationfit.C+.
///
...
// MeasurementArea Class describing a measurement area on the calibration target
// plane. Parameters of the measurement areas are position and side length
// as well as the thickness, density, atomic number and atomic mass of the
// measurement region
...
private:
  ...
  int run_min; // Minimal run number of data, which contains the specified
               // material parameters
  int run_max; // Maximal run number of data, which contains the specified
               // material parameters
  int max_angles; // Maximal number of scattering angles in the scattering angle
                  // distribution

public:
  // Constructors

  MeasurementArea(double, double, double, double, double, double, double,
                  double, double, int, int, int); // Constructor

  // Reading parameters of the measurement area

  double Get_u_min() {
    return center_u - 0.5 * length_u;
  } // Return min u value of MA (mm)
  double Get_u_max() {
    return center_u + 0.5 * length_u;
  } // Return max u value of MA (mm)
  double Get_v_min() {
    return center_v - 0.5 * length_v;
  } // Return min v value of MA (mm)
  double Get_v_max() {
    return center_v + 0.5 * length_v;
  }                                          // Return max v value of MA (mm)
  ...
  double Get_u_length() {
    return length_u;
  } // Return the side length (in u direction) of MA (mm)
  double Get_v_length() {
    return length_v;
  } // Return the side length (in v direction) of MA (mm)
...
MeasurementArea::MeasurementArea(double ucenter, double vcenter, double ulength,
                                 double vlength, double thick,
                                 double X0constant, double dens,
                                 double atom_num, double atom_mass,
                                 int run_minimum, int run_maximum,
                                 int angle_maximum) {
  center_u = ucenter; // mm
  center_v = vcenter; // mm
  length_u = ulength; // mm
...
// Class describing the complete calibration grid. The grid consists of a set of
// holes with a specific radiation length.
// This class can be used to described the aluminium target which was used
// during the March and November 2015 PXD test beams.
// The second constructor can be used to model this exact aluminium target
class Grid {
private:
  // Declaration of parameters

  std::vector<MeasurementArea>
      m_MeasurementAreas; // Declaration the holes in the grid

public:
  // Constructor
  Grid(TEnv *); // Create set of measurement areas from cfg file

  const std::vector<MeasurementArea> &GetMeasurementAreas() const {
    return m_MeasurementAreas;
  }
  std::vector<MeasurementArea> &GetMeasurementAreas() {
    return m_MeasurementAreas;
  }
  ...
  void PrintGridParameters() {
    for (size_t i = 0; i < m_MeasurementAreas.size(); i++) {
      ...
      if (i == m_MeasurementAreas.size() - 1)
        cout << endl;
    }
  }

  // Add another measurement area
  // void AddMeasurementArea(MeasurementArea MA)	{
  // MeasurementAreas.push_back(MA); }
};
...
Grid::Grid(TEnv *mEnv) {
  ...
  while (mEnv->GetValue(MAname + ".exist", 0) != 0) {
    ...
    MeasurementArea MA(ucenter, vcenter, ulength, vlength, thickness, X0,
                       density, Z, A, run_min, run_max, max_angle);
  ...
  while (mEnv->GetValue(rectanglename + ".exist", 0) != 0) {
    ...
    if ((mEnv->GetValue(rectanglename + ".usteplength", 0.0) != 0.0) &&
        (mEnv->GetValue(rectanglename + ".vsteplength", 0.0) != 0.0)) {
...
void savehisto(TFile *, TFile *, TString, TString, double, double, double,
               double, int);
void correcthisto(TFile *, TFile *, TString, double, double, double, double);
double *fit(TFile *, TString, Grid, std::vector<double>, double, TString,
            std::vector<bool>);
void shiftbins(TH1F *, double);
// void calibrationfit();
int **GetParameterMapping(int);

// Function, which returns beam momentum value for every point on the target
// plane
// This is necessary because the beam profile at DESY often have beam energy
// gradients in the order of a few MeV/mm
Double_t GetMomentum(double meanvalue, double ugrad, double vgrad, double u,
                     double v) {
  double p;
...
// Highland model of multiple scattering: Simple gaussian with a well defined
// standard deviation depending on X/X0 and the beam energy.
// The overall function describing the kink angle distributions is the Highland
// function convoluted with a gaussian function due to the finite angle
// resolution on the target plane.
// The formula for the Highland standard deviation taken from the Particle Data
// Group Tanabashi et al. 2018  (eq. 33.15)
Double_t highlandfunction(Double_t *x, Double_t *par) {
  ...
  // // This formula should only be used in case X0 is unknown (1-2% deviation
  // from PDG value)
  double X0 = par[15];

  // beam energy, modified by the energy loss due to bremsstrahlung
  // A weighted mean of the average particle energy before and after the
  // material transition is calculated
  // Additionally a linear beam model is employed to account for possible beam
  // energy gradients due to beam test energy selection with dipole magnets
  ...
  double sigma =
      TMath::Sqrt(pow(recoerror, 2) +
                  pow(0.0136 * charge / (p * beta) * TMath::Sqrt(d1 / X0) *
                          (1.0 + 0.038 * TMath::Log(d1 / X0)),
                      2));
...
// Moliere model of multiple scattering: Function also describing the tails of
// multiple scattering distributions. The function depends on material
// properties, which
// can be reduced to X0, the material thickness and the beam energy.
// The overall function describing the kink angle distributions is the Moliere
// function convoluted with a gaussian function due to the finite angle
// resolution on the target plane.
// The formula and calculations are taken from Bethe 1953 "Moliere's Theory of
// Multiple Scattering"
Double_t molierefunction(Double_t *x, Double_t *par) {
  ...
  double log_omega_b1 = 8.215 +
                        log(pow(Z, (-0.6667)) * (arealdensity1 / A) *
                            pow(alpha, 2) / (1.13 + 3.76 * pow(alpha, 2))) /
                            log(10.0);

  // parameter that will be used in the masterformula of the overall angle
  // distribution (thickness1)
  double B1 = calculateB(log_omega_b1);

  // chi_C parameter (dependend on thickness, momentum etc), given in µrad
  double chi_C = 22.9 * charge * Z / (1000.0 * p) *
                 TMath::Sqrt(arealdensity1 / (pow(beta, 2) * A)) * TMath::Pi() /
                 180.0;
  ...
  double f1_values_aid[numpoints_aid] = {
      0.0206,  -0.0246, -0.1336, -0.2440, -0.2953, -0.2630, -0.1622,
      -0.0423, 0.0609,  0.1274,  0.147,   0.142,   0.1225,  0.100,
      0.078,   0.059,   0.045,   0.0316,  0.0194};
  double f2_values_aid[numpoints_aid] = {0.416,  0.299,  0.019,  -0.229, -0.292,
                                         -0.174, 0.010,  0.138,  0.146,  0.094,
                                         0.045,  -0.049, -0.071, -0.064, -0.043,
                                         -0.024, -0.010, 0.001,  0.006};

  //  f1 and f2 values for the total distribution (also negative angles, but
  //  some missing values)
  double f1_values_aid2[numpoints];
  double f2_values_aid2[numpoints];

  //  f1 and f2 values for the total distribution (also negative angles) between
  //  phi=-4.0 and 4.0 with an 0.1 increment
  ...
  // add temporary values to the bins which will be filled with the extrapolated
  // values later
  for (int i = 0; i < numbins; i++) {
    if (i < 3) {
  ...
  // Declaration of the histogram containing the values of the first function
  // given in table 2
  ...
  // Declaration of the histogram containing the overall moliere angle
  // distibution (including reconstruction errors)
  TH1F *h_moliere_conv =
      new TH1F("moliere_conv", "moliere_conv", numbins, low, high);
  ...
  for (int i = 0; i < numbins; i++) {
    ...
    double value =
        2.0 / TMath::Sqrt(TMath::Pi()) *
        TMath::Exp(-1.0 *
                   pow((phi_values_rad[i] / (chi_C * TMath::Sqrt(B1))), 2));
  ...
  // fill the overall distribution with the values calculated from gaussian f1
  // and f2
  for (int i = 0; i < numbins; i++) {
    // Bin number corresponding to the current phi value
    int bin_No = i + 1;

    // value of the overall function computed from the other functions
    // (thickness 1)
    double total_value1 = h_gaus->GetBinContent(bin_No) +
                          h_f1_table->GetBinContent(bin_No) / B1 +
                          h_f2_table->GetBinContent(bin_No) / (B1 * B1);
  ...
  // Numerical convolution of moliere density and reco error gaussian function
  // (sum over all bins of the histogram)
  // at a certain point defined by j loop
  for (int i = 0; i < numbins; i++) {
    // current value of the sum
    convolutionintegral +=
        h_total1->GetBinContent(i + 1) *
        TMath::Gaus(phi_values_rad[i] - x[0], 2 * par[14], recoerror);
  }
...
// definition of shared parameters like the parameter numbers of each fit
// function and
...
struct GlobalEstimator {
  std::vector<ROOT::Math::IMultiGenFunction *> &getEstimatorVec() {
    return fEstimatorVec;
  }
...
int **GetParameterMapping(int numfuncs) {
  ...
  for (int i = 0; i < numfuncs; i++) {
    ...
    for (int j = 0; j < num_localparameters; j++) {
      ...
      // There are basically 2 cases: The first function has the Parameters
      // 0-15,
      //								the
      //other function have a new Parameter number at the 3rd parameter
      //(density), the 4th parameter (Z), the 5th parameter (A)
      //								the 6th
      //Parameter (thickness of target material), the 9th Parameter (~#tracks),
      //the 10th parameter (u coordinate), the 11th parameter (v coordinate),
      //								the 14th
      //parameter (mean value) and the 15th parameter (X0 value)
      ...
      if (i == 0) {
        ...
        if ((j != 3) && (j != 4) && (j != 5) && (j != 6) && (j != 9) &&
          ...
          ipar[i][j] = num_localparameters + (i - 1) * newparsperfunction;
        else if (j == 4)
          ipar[i][j] = num_localparameters + 1 + (i - 1) * newparsperfunction;
        else if (j == 5)
          ipar[i][j] = num_localparameters + 2 + (i - 1) * newparsperfunction;
        else if (j == 6)
          ipar[i][j] = num_localparameters + 3 + (i - 1) * newparsperfunction;
        else if (j == 9)
          ipar[i][j] = num_localparameters + 4 + (i - 1) * newparsperfunction;
        else if (j == 10)
          ipar[i][j] = num_localparameters + 5 + (i - 1) * newparsperfunction;
        else if (j == 11)
          ipar[i][j] = num_localparameters + 6 + (i - 1) * newparsperfunction;
        else if (j == 14)
          ipar[i][j] = num_localparameters + 7 + (i - 1) * newparsperfunction;
        else
          ipar[i][j] = num_localparameters + 8 + (i - 1) * newparsperfunction;
      }
      cout << "Parameter mapping: Global parameter number of local parameter "
           << j << " in fit function " << i << " is " << ipar[i][j] << endl;
    }
  }
...
double calculateB(double log_omega_b) {
  ...
  // Iniatialize B value as mean between starting values of lower and upper B
  // bound
  double B = (Bmin + Bmax) / 2.0;

  // Calculate difference between measured omega b and omega b(B) with this
  // starting B value
  double difference;
  difference = pow(10, (log_omega_b)) - 1.167 * exp(B) / B;

  // For loop used to minimize difference between measured omega b and omega
  // b(B)
  for (Int_t i = 1; sqrt(pow(difference, 2)) > 1E-8; i++) {
    // If the difference is larger than 0 the current B value is too small,
    // therefore the lower bound of B (Bmin)
    // is now set to be the current X/X0 value
    if (difference > 0) {
      Bmin = B;
    }

    // If the difference is smaller than 0 the current B value is too large,
    // therefore the upper bound of B (Bmax)
    // is now set to be the current X/X0 value
    else {
      Bmax = B;
    }
...
double DetermineFitrange(TH1F *histo, double rangevalue) {
  ...
  cout << "Selected range parameter: " << rangevalue
       << " -> Fit range up to y=1/(" << rangevalue << "*e)" << endl;
  ...
  // In case the second fit failed as well, simply use the RMS of the histo in
  // the 3 sigma range
  if (!fitr->IsValid()) {
    ...
    sigma = histo->GetRMS();
    cout << "Fall back fit of angle distribution failed with status: " << fitr
         << "! Just use RMS of histogram." << endl;
  }
...
double getanglerecovar(TFile *file) {
  // TTree in input root file, that contains the MSC projected angle
  // distributions and reconstruction error distribution
  file->cd("");
  ...
  double recovar_error =
      sqrt(recovar1_error * recovar1_error + recovar2_error * recovar2_error) /
      2.0;

  // The recovar error shouldn't be too large!
  cout << endl
       << "Angle Reconstruction Variance: 	" << recovar << " +/- "
       << recovar_error << "rad^2" << endl;
...
// Function to save the projected angle histograms of different regions in the
// u-v plane, the region lies within the given
// u and v min and max values.
void savehisto(std::vector<TString> inputfiles, TFile *file2, TString histoname,
               int numbins, double histo_range,
               std::vector<double> cutparameters_position,
               std::vector<int> cutparameters_run,
               std::vector<int> cutparameters_vertex_multiplicity,
               int max_angles, int correctmean, TString nametag) {
  ...
  for (size_t ifile = 0; ifile < inputfiles.size(); ifile++) {
    ...
    if ((cutparameters_run.at(0) < 0 && cutparameters_run.at(1) < 0) ||
        (runnumber >= cutparameters_run.at(0) &&
         runnumber <= cutparameters_run.at(1))) {
  ...
  TString areacuts;
  areacuts.Form("(u>%.2f)&&(u<%.2f)&&(v>%.2f)&&(v<%.2f)",
                cutparameters_position.at(0), cutparameters_position.at(1),
                cutparameters_position.at(2), cutparameters_position.at(3));

  // Draw histogram of the first scattering angle in the given u and v range and
  // save it
  ...
  tmp_anglehisto[0] =
      new TH1F("theta1_histo", "theta1_histo", numbins, -limits, limits);
  tmp_anglehisto[1] =
      new TH1F("theta2_histo", "theta2_histo", numbins, -limits, limits);
  ...
  for (size_t ifile = 0; ifile < relevantfiles.size(); ifile++) {
    ...
    if (correctmean == 1) {
      // Get histogram
      TH1F *histogram1 =
          (TH1F *)file2->Get("grid/raw/theta1_uncorrected_" + histoname);
      TH1F *histogram2 =
          (TH1F *)file2->Get("grid/raw/theta2_uncorrected_" + histoname);
      ...
      limits =
          histo_range / 2.0 * (histogram1->GetRMS() + histogram2->GetRMS());
    }

    for (int ientry = 0; ientry < msc_tree->GetEntries(); ientry++) {

      if (ientry % 100000 == 0)
        cout << "Tree entry " << ientry << endl;
      ...
      u_condition = (u > cutparameters_position.at(0)) &&
                    (u < cutparameters_position.at(1));
      v_condition = (v > cutparameters_position.at(2)) &&
                    (v < cutparameters_position.at(3));

      if (cutparameters_run.at(0) > -1 && cutparameters_run.at(1) > -1)
        run_condition = (RunNo >= cutparameters_run.at(0)) &&
                        (RunNo <= cutparameters_run.at(1));
      else if (cutparameters_run.at(0) > -1)
        run_condition = (RunNo >= cutparameters_run.at(0));
      else if (cutparameters_run.at(1) > -1)
        run_condition = (RunNo <= cutparameters_run.at(1));

      if (msc_tree->GetBranchStatus("vertex_multiplicity"))
        vertex_multiplicity_condition =
            (vertex_multiplicity >= cutparameters_vertex_multiplicity.at(0)) &&
            (vertex_multiplicity <= cutparameters_vertex_multiplicity.at(1));

      if (u_condition && v_condition && run_condition &&
          vertex_multiplicity_condition) {
        tmp_anglehisto[0]->Fill(theta1 - mean1);
        tmp_anglehisto[1]->Fill(theta2 - mean2);
      }

      if ((tmp_anglehisto[0]->GetEntries() >= max_angles) && (max_angles > -1))
        break;
    }
  }
  ...
  /*
          // Shift the bins of the histogram to reduce the offset, but only in
     case the offset is larger than the histogram bin size
  ...
  */
  ...
  if (correctmean == 1)
    tmp_anglehisto[0]->Write("theta1_" + nametag + histoname);
  else
    tmp_anglehisto[0]->Write("theta1_" + nametag + histoname);

  // Write histogram to disk
  if (correctmean == 1)
    tmp_anglehisto[1]->Write("theta2_" + nametag + histoname);
  else
    tmp_anglehisto[1]->Write("theta2_" + nametag + histoname);

  // Write histogram to disk
  if (correctmean == 1)
    h->Write("sumhisto_" + nametag + histoname);
  else
    h->Write("sumhisto_" + nametag + histoname);
...
// Function to fit the MSC angle histograms simultaneously, it returns a pointer
// to the fit results
double *fit(TFile *file, Grid grid, std::vector<double> beamoptions,
            double recoerr, TString model, std::vector<bool> fitoptions,
            double rangevalue) {
  ...
  for (size_t i = 0; i < num_fitfunctions; i++) {
    ...
    fithistogramsum =
        (TH1F *)histogramsum->Clone("sumhisto_" + histoname + "_fit");
    histo_vec.push_back(fithistogramsum);

    file->cd("grid/fit/");
  }

  // The fit range depend on the model: In Case of the moliere model the
  // fitrange can be a little larger
  ...
  // loop for definition of the fit functions, the fitrange is determined for
  // every one of them
  for (size_t i = 0; i < num_fitfunctions; i++) {
    ...
    if (model == "moliere") {
      // Use Gaussian function with width corresponding to the calibrated angle
      // resolution, if material is only air or extremely thin
      // Also in this case the fit range is set to be a little smaller
      if (grid.GetMeasurementAreas().at(i).Get_thickness() < 0.0002)
        fitFcn =
            new TF1(fctname, "[9]*TMath::Gaus(x,0.0*[0]*[1]*[2]*[3]*[4]*[5]*[6]"
                             "*[10]*[11]*[12]*[13]*[15]*[16]+[14],[7]*[8])",
                    -fitrange, fitrange);

      // Use Moliere model in case the material is not just air
      else
        fitFcn = new TF1(fctname, molierefunction, -fitrange, fitrange,
                         num_localparameters);
    }
      ...
      // Use Highland model in case the material is not just air
        ...
        fitFcn = new TF1(fctname, highlandfunction, -fitrange, fitrange,
                         num_localparameters);
    }
    ...
    cout << "Fitrange at d=" << grid.GetMeasurementAreas().at(i).Get_thickness()
         << " mm: " << fitrange << " rad" << endl
         << endl;
  ...
  for (size_t i = 0; i < num_fitfunctions; i++) {
    ...
    ROOT::Math::WrappedMultiTF1 *wf_entry =
        new ROOT::Math::WrappedMultiTF1(*fitFcn_vec.at(i), 1);
    ...
    if (Use_LogLikelihoodfit) {
      ROOT::Fit::PoissonLLFunction *log_entry =
          new ROOT::Fit::PoissonLLFunction(*data_entry, *wf_entry);
      globalestimator.getEstimatorVec().push_back(log_entry);
    } else {
      ROOT::Fit::Chi2Function *chi2_entry =
          new ROOT::Fit::Chi2Function(*data_entry, *wf_entry);
  ...
  // Number of global parameters: There are num_fitfunctions fit functions with
  // three new parameters each: Scale, u coordinate, vcoordinate, thickness
  // Then there are 8 parameters, which are used in all fit functions (beam
  // energy calibration factor, angle calibration factor, material properties,
  // etc)
  static const int num_globalparameters =
      num_fitfunctions * newparsperfunction +
      (num_localparameters - newparsperfunction);
  ...
  double aid_array[num_localparameters] = {
      BE_mean,
      z,
      mass,
      grid.GetMeasurementAreas().at(0).Get_density(),
      grid.GetMeasurementAreas().at(0).Get_Z(),
      grid.GetMeasurementAreas().at(0).Get_A(),
      grid.GetMeasurementAreas().at(0).Get_thickness(),
      recoerr,
      lambda_startvalue,
      700.0,
      grid.GetMeasurementAreas().at(0).Get_u_center(),
      grid.GetMeasurementAreas().at(0).Get_v_center(),
      BE_ugrad,
      BE_vgrad,
      0.0,
      grid.GetMeasurementAreas().at(0).Get_X0(),
      epsilon};
  for (int i = 0; i < num_localparameters; i++)
    par0[i] = aid_array[i];
  ...
  for (size_t i = 1; i < num_fitfunctions; i++) {
    par0[num_localparameters + (i - 1) * newparsperfunction] =
        grid.GetMeasurementAreas().at(i).Get_density();
    par0[num_localparameters + 1 + (i - 1) * newparsperfunction] =
        grid.GetMeasurementAreas().at(i).Get_Z();
    par0[num_localparameters + 2 + (i - 1) * newparsperfunction] =
        grid.GetMeasurementAreas().at(i).Get_A();
    par0[num_localparameters + 3 + (i - 1) * newparsperfunction] =
        grid.GetMeasurementAreas().at(i).Get_thickness();
    par0[num_localparameters + 4 + (i - 1) * newparsperfunction] = 700;
    par0[num_localparameters + 5 + (i - 1) * newparsperfunction] =
        grid.GetMeasurementAreas().at(i).Get_u_center();
    par0[num_localparameters + 6 + (i - 1) * newparsperfunction] =
        grid.GetMeasurementAreas().at(i).Get_v_center();
    par0[num_localparameters + 7 + (i - 1) * newparsperfunction] = 0.0;
    par0[num_localparameters + 8 + (i - 1) * newparsperfunction] =
        grid.GetMeasurementAreas().at(i).Get_X0();
  }
  ...
  // Fix momentum offset?
  if (fixmomentumoffset)
    fitter.Config().ParSettings(0).Fix();

  // Fix the beam momentum gradient parameters?
  if ((fixmomentumugradient))
    fitter.Config().ParSettings(12).Fix();
  if ((fixmomentumvgradient))
    fitter.Config().ParSettings(13).Fix();

  // Limit the beam momentum gradient parameters, they should be small anyway (<
  // 10MeV/mm)
  // and we dont want to see negative beam energies at any position on the
  // target plane
  ...
  // fix X0, density, A, Z, thickness and coordinate parameters for all
  // fitfunctions
  for (size_t i = 1; i < num_fitfunctions; i++) {
    fitter.Config()
        .ParSettings(num_localparameters + (i - 1) * newparsperfunction)
        .Fix();
    fitter.Config()
        .ParSettings(num_localparameters + 1 + (i - 1) * newparsperfunction)
        .Fix();
    fitter.Config()
        .ParSettings(num_localparameters + 2 + (i - 1) * newparsperfunction)
        .Fix();
    fitter.Config()
        .ParSettings(num_localparameters + 3 + (i - 1) * newparsperfunction)
        .Fix();
    fitter.Config()
        .ParSettings(num_localparameters + 5 + (i - 1) * newparsperfunction)
        .Fix();
    fitter.Config()
        .ParSettings(num_localparameters + 6 + (i - 1) * newparsperfunction)
        .Fix();
    fitter.Config()
        .ParSettings(num_localparameters + 8 + (i - 1) * newparsperfunction)
        .Fix();
  }

  if (fixlambda)
    fitter.Config().ParSettings(8).Fix(); // fix lambda?
  ...
  for (size_t i = 1; i < num_fitfunctions; i++) {
    fitter.Config()
        .ParSettings(num_localparameters + 4 + (i - 1) * newparsperfunction)
        .SetLimits(10.0, 200000.0);
    fitter.Config()
        .ParSettings(num_localparameters + 7 + (i - 1) * newparsperfunction)
        .SetLimits(-0.0001, +0.0001);
  }
  ...
  fitter.FitFCN(num_globalparameters, globalestimator, 0, datasize,
                !Use_LogLikelihoodfit);
  ...
  for (size_t i = 0; i < num_fitfunctions; i++) {
    ...
    for (int iname = 0; iname < num_localparameters; iname++)
      fitfunc->SetParName(iname, name[iname]);
  }

  cout << "Plot the fit results, print them in terminal and save them to "
          "histogram"
       << endl;
  ...
  for (size_t i = 0; i < num_fitfunctions; i++) {
    ...
    cout << "Chi2 value for this individual fit is: "
         << histo_vec.at(i)->Chisquare(fitFcn_vec.at(i), "R") << endl;
    cout << "Number of degrees of freedom is: " << fitFcn_vec.at(i)->GetNDF()
         << endl;
    cout << "--------------------------" << endl;
  }
  ...
  for (int i = 0; i < TMath::Ceil(double(num_fitfunctions) / 4.0); i++) {
    ...
    for (size_t j = 0; j < 4; j++) {
      ...
      if (((4 * i) + j) < num_fitfunctions) {
        Title.Form("Area %lu: d=%fmm", (4 * i) + j,
                   grid.GetMeasurementAreas().at((4 * i) + j).Get_thickness());
        histo_vec.at((4 * i) + j)->SetTitle(Title);
        histo_vec.at((4 * i) + j)->Draw();
        cout << "fitfunction " << (4 * i) + j << " of " << num_fitfunctions
             << endl;
      }
    }
  ...
  // Check whether a fit with the determined calibration parameters yields the
  // correct radiation length value for all the
  ...
  // Declaration of another set of fit functions, these will be used for the
  // self consistency check
  std::vector<TF1 *> fitFcn_vec2;

  TH1F *h_d =
      new TH1F("h_d", "h_d", num_fitfunctions, 0.5, num_fitfunctions + 0.5);
  TH1F *h_d_true = new TH1F("h_d_true", "h_d_true", num_fitfunctions, 0.5,
                            num_fitfunctions + 0.5);
  ...
  for (size_t i = 0; i < TMath::Ceil(double(num_fitfunctions) / 4.0); i++) {
    ...
    for (size_t j = 0; j < 4; j++) {
      ...
      if (((4 * i) + j) < num_fitfunctions) {
        ...
        TFitResultPtr fitr = histo_vec.at((4 * i) + j)
                                 ->Fit(fitFcn_vec2.at((4 * i) + j), fitoption);
        if (fitr != 0) {
          cout << "Fit of angle distribution failed with status: " << fitr
               << endl;
          cout << "Repeat fit " << endl;
          histo_vec.at((4 * i) + j)
              ->Fit(fitFcn_vec2.at((4 * i) + j), fitoption);
        ...
        histo_vec.at((4 * i) + j)->Draw();
        cout << "fitfunction " << (4 * i) + j + 1 << " of " << num_fitfunctions
             << endl;

        h_d->SetBinContent((4 * i) + j + 1,
                           fitFcn_vec2.at((4 * i) + j)->GetParameter(6));
        h_d->SetBinError((4 * i) + j + 1,
                         fitFcn_vec2.at((4 * i) + j)->GetParError(6));
        h_d_true->SetBinContent(
            (4 * i) + j + 1,
            grid.GetMeasurementAreas().at((4 * i) + j).Get_thickness());
      }
    }
  ...
  TH1F *resultshist =
      new TH1F("resultshist", "results of calibration", 4, 1, 4);
...
void GetInputFiles(std::vector<TString> &inputfiles, const char *dirname = ".",
                   const char *ext = "run") {
...
// Function used to get the angle distributions of tracks crossing certain
// regions in the u-v measurement plane,
// perform a moliere fit on the distributions to estimate the calibrationfactors
// mu and lambda. Usually this script is used on
// measurement data of a plane with a precisely known material distribution (
// for example a aluminum grid with a set
...
int main(int, char **) {
  ...
  // Integer determining, whether a offset correction is applied to the angular
  // distributions
  // 1: correction
  ...
  // BE: beam energy (GeV), z: charge of beam particle (e), mass: mass of beam
  // particle (GeV),
  // d_layer: thickness per layer(mm), num_layers: total number of layers,
  // recoerr: angle reconstruction error (rad)
  ...
  double BE_ugrad_default = mEnv->GetValue(
      "momentumugradient", 99.0); // energy slope in GeV/mm in u direction
  double BE_vgrad_default = mEnv->GetValue(
      "momentumvgradient", 99.0); // energy slope in GeV/mm in u direction

  double epsilon = mEnv->GetValue(
      "epsilon", 0.0); // weight of the energy loss formula (see fit models)

  int vertexmultiplicitymin = mEnv->GetValue(
      "vertexmultiplicitymin", 1); // energy slope in GeV/mm in u direction
  int vertexmultiplicitymax = mEnv->GetValue(
      "vertexmultiplicitymax", 1); // energy slope in GeV/mm in u direction
  ...
  // Definition of the measurement areas can be done in two ways. Either a fixed
  // 3 x 3 alu grid with a regular thickness increase between grid points and
  // fixed positions or
  // 12 completely independent measurement areas with completely unreleated
  ...
  for (size_t i = 0; i < num_fitfunctions; i++) {
    ...
    cutparameters_position.push_back(
        grid.GetMeasurementAreas().at(i).Get_u_min());
    cutparameters_position.push_back(
        grid.GetMeasurementAreas().at(i).Get_u_max());
    cutparameters_position.push_back(
        grid.GetMeasurementAreas().at(i).Get_v_min());
    cutparameters_position.push_back(
        grid.GetMeasurementAreas().at(i).Get_v_max());
    ...
    // Save the angle histograms of the current measurement area to the root
    // file
    cout << endl << "Correct angle distribution offsets..." << endl;
    TString nametag = "uncorrected_";
    savehisto(inputfiles, rootfile, histoname, numbins, histo_range,
              cutparameters_position, cutparameters_run,
              cutparameters_vertex_multiplicity, maxangles, 0, nametag);
    cout << endl << "Write histos..." << endl;
    nametag = "";
    savehisto(inputfiles, rootfile, histoname, numbins, histo_range,
              cutparameters_position, cutparameters_run,
              cutparameters_vertex_multiplicity, maxangles, correctmean,
              nametag);
  ...
  // Check whether there are entries, of this is the case use these entries as
  // starting values

  double lambda_start =
      mEnv_res->GetValue("lambda_start", lambda_start_default);
  ...
  // lambda and epsilon are not really a beam parameters, but this is the only
  // place this fits in
  ...
  cout << endl
       << "Beam particle mean energy start value is			"
          "	"
       << BE_mean << " GeV" << endl;
  cout << "Beam particle energy u gradient start value is		"
       << BE_ugrad << " GeV/mm" << endl;
  cout << "Beam particle energy v gradient start value is		"
       << BE_vgrad << " GeV/mm" << endl;
  cout << "Beam particle lambda		 start value is			"
          "	"
       << lambda_start << endl
       << endl;

  if (fixlambda)
    cout << "Calibration factor Lambda will not be calibrated" << endl;
  else
    cout << "Calibration factor Lambda will be calibrated" << endl;

  if (fix_offset)
    cout << "Beam particle mean energy will not be calibrated" << endl << endl;
  else
    cout << "Beam particle mean energy will be calibrated" << endl << endl;

  cout << "Using " << model
       << " fit model during calibration and self-consistency check!" << endl;
  if (Use_loglikelihood_estimator)
    cout << "Using log-likelihood estimator fit during calibration and "
            "self-consistency check!"
         << endl
         << endl;
  else
    cout << "Using chi2 estimator fit during calibration and self-consistency "
            "check!"
         << endl
         << endl;

  double *iresults =
      fit(rootfile, grid, beamoptions, recoerr, model, fitoptions, rangevalue);

  cout << endl
       << " The lambda calibration factor is: " << iresults[0] << " +/- "
       << iresults[1] << endl;
  cout << " The mean beam energy at (0,0) is: " << iresults[2] << " +/- "
       << iresults[3] << "GeV" << endl;
  cout << " The kappa calibration factor is: " << BE_mean / iresults[2]
       << " +/- " << iresults[3] * BE_mean / (iresults[2] * iresults[2])
       << endl;
  cout << " The BE gradient in u direction is: " << iresults[4] << " +/- "
       << iresults[5] << "GeV/mm" << endl;
  cout << " The BE gradient in v direction is: " << iresults[6] << " +/- "
       << iresults[7] << "GeV/mm" << endl;
