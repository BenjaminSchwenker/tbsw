...
namespace depfet {
    ...
    *  specified in the geometry. Multiple hit collections can be given as 
    *  as input. The processor outputs a single track collection which can 
    *  be used in downstream processors. The other output is a collection of 
    *  all hits that have not been used for any of the final tracks. 
    *
    ...
    */
    ...
    class FastTracker : public marlin::Processor {
     ...
     protected:
      ...
      static bool check_incompatible( const TBTrack& trk1, const TBTrack& trk2 );
      
      //! Is track trk1 better than trk2
      static bool compare_tracks( const TBTrack& trk1, const TBTrack& trk2 );
      
      //! Mark hits in track as used
      static void mark_hits( const TBTrack& trk, std::vector<std::vector<int>>&  usedIDs );
