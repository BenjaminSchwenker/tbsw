...
void TrackFitDQM::end()
{
  ...
  for (int ipl=0 ; ipl < TBDetector::GetInstance().GetNSensors(); ipl++) {
    ...
    _overviewHistoMap["hxshift_diff"s]->SetBinContent(ipl+1,pos_f[0]);
    _overviewHistoMap["hyshift_diff"s]->SetBinContent(ipl+1,pos_f[1]);
    _overviewHistoMap["hzshift_diff"s]->SetBinContent(ipl+1,pos_f[2]);
    _overviewHistoMap["hxrot_diff"s]->SetBinContent(ipl+1,alpha_f);
    _overviewHistoMap["hyrot_diff"s]->SetBinContent(ipl+1,beta_f);
    _overviewHistoMap["hzrot_diff"s]->SetBinContent(ipl+1,gamma_f);
...
{   
  ...
  for (int ipl=0 ; ipl < nSens; ipl++) {
    ...
    _histoMap2D["hhitmap_sensor"s] = new TH2D(histoName.c_str(), "" ,nbins, minU, +maxU, nbins, minV, +maxV);
