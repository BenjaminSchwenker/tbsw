#ifndef EUDAQINPUT_INCLUDED_Event
#define EUDAQINPUT_INCLUDED_Event
...
#define EUDAQINPUT_DECLARE_EVENT(type)           \
  public:                                   \
    static unsigned eudaqinput_static_id();      \
    virtual unsigned get_id() const {       \
      return eudaqinput_static_id();             \
    }                                       \
  private:                                  \
    static const int EUDAQINPUT_DUMMY_VAR_DONT_USE = 0

#define EUDAQINPUT_DEFINE_EVENT(type, id)       \
  unsigned type::eudaqinput_static_id() {       \
    static const unsigned id_(id);         \
    return id_;                            \
  }                                        \
  namespace _eudaqinput_dummy_ {                \
    static eudaqinput::RegisterEventType<type> eudaqinput_reg;	\
  }                                        \
  static const int EUDAQINPUT_DUMMY_VAR_DONT_USE = 0
...
namespace eudaqinput {
  ...
  public:
    ...
    Event & SetTag(const std::string & name, const T & val) {
      return SetTag(name, eudaqinput::to_string(val));
    }
    ...
    T GetTag(const std::string & name, T def) const {
      return eudaqinput::from_string(GetTag(name), def);
    }
  ...
  struct RegisterEventType {
    RegisterEventType() {
      EventFactory::Register(T_Evt::eudaqinput_static_id(), &factory_func);
    }
...
#endif // EUDAQINPUT_INCLUDED_Event
