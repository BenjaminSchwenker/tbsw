...
namespace eudaqinput {
    ...
    : m_file(0), m_filebytes(0), m_wprot(wprot)
  {
    if (!overwrite) {
      ...
      if (fd) {
        fclose(fd);
        EUDAQINPUT_THROWX(FileExistsException, "File already exists: " + fname);
      }
    }
    m_file = fopen(fname.c_str(), "wb");
    if (!m_file) EUDAQINPUT_THROWX(FileNotFoundException, "Unable to open file: " + fname);
  ...
  void FileSerializer::Serialize(const unsigned char * data, size_t len) {
    ...
    m_filebytes += written;
    if (written != len) {
      EUDAQINPUT_THROWX(FileNotFoundException, "Error writing to file: " + to_string(errno) + ", " + strerror(errno));
    }
  }
  ...
  size_t FileDeserializer::FillBuffer(size_t min) {
    ...
    while (read < min) {
      if (feof(m_file) && m_faileof) {
        ...
        EUDAQINPUT_THROWX(FileReadException, "Error reading from file: " + to_string(err));
