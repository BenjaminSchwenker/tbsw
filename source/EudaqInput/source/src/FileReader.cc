...
namespace eudaqinput {
  ...
  struct FileReader::eventqueue_t {
    struct item_t {
      ...
      eudaqinput::DetectorEvent * event;
    ...
    eudaqinput::DetectorEvent * popevent() {
    ...
    std::list<item_t>::const_iterator iter(size_t producer, int offset = 0) const {
      ...
      for (int i = 0; i <= offset; ++i) {
        if (it == items.begin()) EUDAQINPUT_THROWX(FileReadException, "Bad offset in ResyncTLU routine");  
        --it;
      }
      return it;
    }
    ...
    const eudaqinput::Event & getevent(size_t producer, int offset = 0) const {
  ...
  namespace {
    ...
    static bool SyncEvent(FileReader::eventqueue_t & queue, FileDeserializer & des, int ver, eudaqinput::Event * & ev) {
        ...
        if (queue.firstid == (unsigned)-1 && !queue.getevent(0).IsBORE()) {
          for (size_t i = 0; i < queue.producers(); ++i) {
            if (queue.getevent(i).get_id() != TLUID) {
              ...
              if (queue.firstid <= 1) {
                //EUDAQINPUT_INFO("First TLU id detected as " + to_string(queue.firstid));
              } else {
                //EUDAQINPUT_WARN("First TLU id detected as " + to_string(queue.firstid) + " (should be 0 or 1)");
              }
              break;
            }
          }
	...
	if (dbg) std::cout << "Event " << eventnum
	  ...
	  if (dbg) std::cout << ", ok" << std::endl;
        ...
        if (triggerid != 0 && triggerid == queue.lastid) {
          //EUDAQINPUT_WARN("Trigger ID " + to_string(triggerid) + " is repeated for whole event " + to_string(eventnum));      
        }
        if (hasrepeat) {
          for (size_t i = 0; i < queue.producers(); ++i) {
            unsigned tid = queue.getid(i);
            if (tid == queue.lastid) {
              ...
              //EUDAQINPUT_INFO("Discarded repeated ID (" + to_string(triggerid) + ") in event " + to_string(eventnum));
        ...
        if (hasother) {
          ...
          if (istlumiss) {
            ...
            //EUDAQINPUT_INFO("Skipped TLU event detected, discarding rest of event " + to_string(eventnum));
            for (size_t i = 0; i < queue.producers(); ++i) {
              if (queue.getevent(i).get_id() != TLUID) {
                queue.discardevent(i);
              }
          ...
          if (isothermiss) {
            ...
            //EUDAQINPUT_INFO("Skipped event detected, discarding rest of event " + to_string(eventnum));
            for (size_t i = 0; i < queue.producers(); ++i) {
	  ...
	  if (dbg) std::cout << ", " << to_string(nums) << std::endl;
          //EUDAQINPUT_WARN("Unexpected tid in event number " + to_string(eventnum) + ": " + to_string(nums));
        } else {
	  if (dbg) std::cout << ", hmm" << std::endl;
        ...
        for (size_t i = 0; i < queue.producers(); ++i) {
          ...
          if (tid == 0) {
            ...
            if (tid1 == triggerid) {
              //EUDAQINPUT_INFO("Discarded extra 'zero' in event " + to_string(eventnum));
              queue.discardevent(i);
            } else if (tid1 == triggerid1) {
              //EUDAQINPUT_DEBUG("Detected 'zero' in event " + to_string(eventnum));
            } else if (tid1 == 0) {
              ...
              if (tid2 == triggerid) {
                //EUDAQINPUT_INFO("Discarded two extra 'zero's in event " + to_string(eventnum));
                queue.discardevent(i);
                queue.discardevent(i);
              } else if (tid2 == triggerid2) {
                //EUDAQINPUT_DEBUG("Detected double 'zero' in event " + to_string(eventnum));
              } else if (tid2 == triggerid1) {
                //EUDAQINPUT_WARN("Ambiguous double zero in event " + to_string(eventnum) + ", discarding whole event plus zero");
                queue.discardevent(i);
                queue.popevent();
                doskip = true;
                break;
              } else if (tid2 == 0) {
                //EUDAQINPUT_WARN("Three consecutive 'zero's in event " + to_string(eventnum) + ", discarding one event.");
                queue.popevent();
                doskip = true;
                break;
              }
            } else {
              EUDAQINPUT_THROWX(FileReadException, "Unable to synchronize at event " + to_string(eventnum) +
      ...
      //EUDAQINPUT_WARN("Unable to synchronize after " + to_string(MAXTRIES) + " events at event " + to_string(eventnum));
      return false;
    }
