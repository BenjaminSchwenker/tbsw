...
namespace eudaqinput {
  ...
  void WriteStringToFile(const std::string & fname, const std::string & val) {
    ...
    if (!file.is_open())  EUDAQINPUT_THROWX(FileReadException, "Unable to open file " + fname + " for writing");
    file << val << std::endl;
    if (file.fail()) EUDAQINPUT_THROWX(FileReadException, "Error writing to file " + fname);
  }
  ...
  std::string ReadLineFromFile(const std::string & fname) {
    ...
    if (file.is_open()) {
      ...
      if (file.fail()) {
        EUDAQINPUT_THROWX(FileReadException, "Error reading from file " + fname);  
      }
    }
    return result;
  }
